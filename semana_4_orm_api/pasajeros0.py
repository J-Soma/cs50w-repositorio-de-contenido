...
def main():
    ...
    vuelos = db.execute("SELECT id, origen, destino, duracion FROM Vuelo").fetchall()
    for vuelo in vuelos:
        print(f"Vuelo {vuelo.id}: {vuelo.origen} hacia {vuelo.destino}, {vuelo.duracion} minutos.")

    # Solicitar al usuario escoger un vuelo.
    id_vuelo = int(input("\nID Vuelo: "))
    vuelo = db.execute("SELECT origen, destino, duracion FROM Vuelo WHERE id = :id",
    ...
    # Listar pasajeros.
    pasajeros = db.execute("SELECT nombre FROM Pasajero WHERE id_vuelo = :id_vuelo",
